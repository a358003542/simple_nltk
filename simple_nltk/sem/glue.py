...
import simple_nltk
from simple_nltk.internals import Counter
from simple_nltk.tag import UnigramTagger, BigramTagger, TrigramTagger, RegexpTagger
...
from simple_nltk.sem import drt
from simple_nltk.sem import linearlogic
...
class GlueDict(dict):
    ...
    def read_file(self, empty_first=True):
        ...
        try:
            contents = simple_nltk.data.load(
                self.filename, format="text", encoding=self.file_encoding
            )
            # TODO: the above can't handle zip files, but this should anyway be fixed in simple_nltk.data.load()
...
class Glue(object):
    def __init__(
        ...
        from simple_nltk import Prover9
    ...
    def dep_parse(self, sentence):
        ...
        if self.depparser is None:
            from simple_nltk.parse import MaltParser
