...
def _join(lst, sep=" ", untag=False):
    ...
    try:
        ...
        from simple_nltk.tag import tuple2str
...
def in_demo(trace=0, sql=True):
    ...
    query.
    """
    from simple_nltk.corpus import ieer
...
def conllned(trace=1):
    ...
    from simple_nltk.corpus import conll2002
...
def ne_chunked():
    print()
    print("1500 Sentences from Penn Treebank, as processed by simple_nltk NE Chunker")
    print("=" * 45)
    ROLE = re.compile(
        r".*(chairman|president|trader|scientist|economist|analyst|partner).*"
    )
    rels = []
    for i, sent in enumerate(simple_nltk.corpus.treebank.tagged_sents()[:1500]):
        sent = simple_nltk.ne_chunk(sent)
...
if __name__ == "__main__":
    import simple_nltk
    from simple_nltk.sem import relextract
