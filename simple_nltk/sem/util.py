...
import codecs
from simple_nltk.sem import evaluate
...
def parse_sents(inputs, grammar, trace=0):
    ...
    :type grammar: simple_nltk.grammar.FeatureGrammar
    :rtype: list(simple_nltk.tree.Tree) or dict(list(str)): list(Tree)
    :return: a mapping from input sentences to a list of ``Tree``s
    """
    # put imports here to avoid circult dependencies
    from simple_nltk.grammar import FeatureGrammar
    from simple_nltk.parse import FeatureChartParser, load_parser
...
def root_semrep(syntree, semkey="SEM"):
    ...
    """
    from simple_nltk.grammar import FeatStructNonterminal
...
def interpret_sents(inputs, grammar, semkey="SEM", trace=0):
    ...
    :rtype: list(list(tuple(simple_nltk.tree.Tree, simple_nltk.sem.logic.ConstantExpression)))
    """
    return [
...
def evaluate_sents(inputs, grammar, model, assignment, trace=0):
    ...
    :rtype: list(list(tuple(simple_nltk.tree.Tree, simple_nltk.sem.logic.ConstantExpression, bool or dict(str): bool)))
    """
    return [
        [
