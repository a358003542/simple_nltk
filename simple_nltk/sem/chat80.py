...
``simple_nltk.sem.evaluate``. The code assumes that the Prolog
input files are available in the simple_nltk corpora directory.
...
class Concept(object):
    ...
    def close(self):
        ...
        """
        from simple_nltk.sem import is_rel
...
def sql_query(dbname, query):
    ...
    try:
        path = simple_nltk.data.find(dbname)
...
def _str2records(filename, rel):
    ...
    """
    recs = []
    contents = simple_nltk.data.load("corpora/chat80/%s" % filename, format="text")
...
def make_valuation(concepts, read=False, lexicon=False):
    ...
    if lexicon:
        read = True
    if read:
        from simple_nltk.sem import Valuation
...
Valuation object for use in the simple_nltk semantics package.
    """
