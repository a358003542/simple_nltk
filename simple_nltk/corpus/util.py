...
class LazyCorpusLoader(object):
    ...
    corpus object before the corpus is loaded.  This allows simple_nltk to
    ...
    simple_nltk data package.  Once they've properly installed the data
    package (or modified ``simple_nltk.data.path`` to point to its location),
    they can then use the corpus object without restarting python.

    :param name: The name of the corpus
    :type name: str
    :param reader_cls: The specific CorpusReader class, e.g. PlaintextCorpusReader, WordListCorpusReader
    :type reader: simple_nltk.corpus.reader.api.CorpusReader
    :param simple_nltk_data_subdir: The subdirectory where the corpus is stored.
    :type simple_nltk_data_subdir: str
    ...
    def __init__(self, name, reader_cls, *args, **kwargs):
        from simple_nltk.corpus.reader.api import CorpusReader

        assert issubclass(reader_cls, CorpusReader)
        self.__name = self.__name__ = name
        self.__reader_cls = reader_cls
        # If simple_nltk_data_subdir is set explicitly
        if "simple_nltk_data_subdir" in kwargs:
            # Use the specified subdirectory path
            self.subdir = kwargs["simple_nltk_data_subdir"]
            # Pops the `simple_nltk_data_subdir` argument, we don't need it anymore.
            kwargs.pop("simple_nltk_data_subdir", None)
        else:  # Otherwise use 'simple_nltk_data/corpora'
            self.subdir = "corpora"
        self.__args = args
    ...
    def __load(self):
        ...
        if TRY_ZIPFILE_FIRST:
            try:
                root = simple_nltk.data.find("{}/{}".format(self.subdir, zip_name))
            except LookupError as e:
                try:
                    root = simple_nltk.data.find("{}/{}".format(self.subdir, self.__name))
                except LookupError:
                    raise e
        else:
            try:
