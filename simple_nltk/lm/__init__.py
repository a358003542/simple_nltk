...
"""
simple_nltk Language Modeling Module.
------------------------------
...
Fortunately, simple_nltk also has a function for that, let's see what it does to the
first sentence.

    >>> from simple_nltk.util import pad_sequence
...
already set while the other arguments remain the same as for `pad_sequence`.
    ...
    >>> from simple_nltk.lm.preprocessing import pad_both_ends
...
simple_nltk once again helpfully provides a function called `everygrams`.
While not the most efficient, it is conceptually simple.


    >>> from simple_nltk.util import everygrams
...
ngrams) and then combine the sentences into one flat stream of words.
    ...
    >>> from simple_nltk.lm.preprocessing import flatten
...
from simple_nltk.lm.models import (
    MLE,
    Lidstone,
    Laplace,
...
from simple_nltk.lm.counter import NgramCounter
from simple_nltk.lm.vocabulary import Vocabulary
