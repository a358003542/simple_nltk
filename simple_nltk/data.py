...
"""
Functions to find and load simple_nltk resource files, such as corpora,
grammars, and saved processing objects.  Resource files are identified
using URLs, such as ``simple_nltk:corpora/abc/rural.txt`` or
``http://simple_nltk.org/sample/toy.cfg``.  The following URL protocols are
supported:
  ...
  - ``simple_nltk:path``: Specifies the file stored in the simple_nltk data
    package at *path*.  simple_nltk will search for these files in the
    directories specified by ``simple_nltk.data.path``.

If no protocol is specified, then the default protocol ``simple_nltk:`` will
be used.
...
import simple_nltk
from simple_nltk.compat import py3_data, add_py3_data
from simple_nltk.internals import deprecated
...
path = []
"""A list of directories where the simple_nltk data package might reside.
   These directories will be checked in order when looking for a
   resource in the data package.  Note that this allows users to
   substitute in their own versions of resources, if they have them
   (e.g., in their home directory under ~/simple_nltk_data)."""

## add pkg resource
from pkg_resources import resource_filename
path.append(resource_filename("simple_nltk", f"data"))

# User-specified locations:
_paths_from_env = os.environ.get("simple_nltk_DATA", str("")).split(os.pathsep)
path += [d for d in _paths_from_env if d]
if "APPENGINE_RUNTIME" not in os.environ and os.path.expanduser("~/") != "~/":
    path.append(os.path.expanduser(str("~/simple_nltk_data")))

if sys.platform.startswith("win"):
    # Common locations on Windows:
    path += [
        os.path.join(sys.prefix, str("simple_nltk_data")),
        os.path.join(sys.prefix, str("share"), str("simple_nltk_data")),
        os.path.join(sys.prefix, str("lib"), str("simple_nltk_data")),
        os.path.join(os.environ.get(str("APPDATA"), str("C:\\")), str("simple_nltk_data")),
        str(r"C:\simple_nltk_data"),
        str(r"D:\simple_nltk_data"),
        str(r"E:\simple_nltk_data"),
    ]
else:
    ...
    path += [
        ...
        os.path.join(sys.prefix, str("share"), str("simple_nltk_data")),
        os.path.join(sys.prefix, str("lib"), str("simple_nltk_data")),
        str("/usr/share/simple_nltk_data"),
        str("/usr/local/share/simple_nltk_data"),
        str("/usr/lib/simple_nltk_data"),
        str("/usr/local/lib/simple_nltk_data"),
    ]
...
def split_resource_url(resource_url):
    ...
    >>> split_resource_url('simple_nltk:home/simple_nltk')
    ('simple_nltk', 'home/simple_nltk')
    >>> split_resource_url('simple_nltk:/home/simple_nltk')
    ('simple_nltk', '/home/simple_nltk')
    >>> split_resource_url('file:/home/simple_nltk')
    ('file', '/home/simple_nltk')
    >>> split_resource_url('file:///home/simple_nltk')
    ('file', '/home/simple_nltk')
    >>> split_resource_url('file:///C:/home/simple_nltk')
    ('file', '/C:/home/simple_nltk')
    """
    protocol, path_ = resource_url.split(":", 1)
    if protocol == "simple_nltk":
        pass
    elif protocol == "file":
        if path_.startswith("/"):
            path_ = "/" + path_.lstrip("/")
...
def normalize_resource_url(resource_url):
    ...
    >>> not windows or normalize_resource_url('file:////C:/dir/file') == 'file:///C:/dir/file'
    True
    >>> not windows or normalize_resource_url('simple_nltk:C:/dir/file') == 'file:///C:/dir/file'
    True
    >>> not windows or normalize_resource_url('simple_nltk:C:\\dir\\file') == 'file:///C:/dir/file'
    True
    >>> windows or normalize_resource_url('file:/dir/file/toy.cfg') == 'file:///dir/file/toy.cfg'
    True
    >>> normalize_resource_url('simple_nltk:home/simple_nltk')
    'simple_nltk:home/simple_nltk'
    >>> windows or normalize_resource_url('simple_nltk:/home/simple_nltk') == 'file:///home/simple_nltk'
    True
    >>> normalize_resource_url('http://example.com/dir/file')
    'http://example.com/dir/file'
    >>> normalize_resource_url('dir/file')
    'simple_nltk:dir/file'
    """
    try:
        protocol, name = split_resource_url(resource_url)
    except ValueError:
        # the resource url has no protocol, use the simple_nltk protocol by default
        protocol = "simple_nltk"
        name = resource_url
    # use file protocol if the path is an absolute path
    if protocol == "simple_nltk" and os.path.isabs(name):
        protocol = "file://"
...
class PathPointer(metaclass=ABCMeta):
    """
    An abstract base class for 'path pointers,' used by simple_nltk's data
...
class BufferedGzipFile(GzipFile):
    """A ``GzipFile`` subclass for compatibility with older simple_nltk releases.
...
def find(resource_name, paths=None):
    ...
    the installation instructions for the simple_nltk downloader.
    ...
    Zip File Handling:
      ...
      - If any element of ``simple_nltk.data.path`` has a ``.zip`` extension,
    ...
    msg = str(
        ...
        "Please use the simple_nltk Downloader to obtain the resource:\n\n"
        "\33[31m"  # To display red text in terminal.
        ">>> import simple_nltk\n"
        ">>> simple_nltk.download('{resource}')\n"
        "\033[0m"
    ).format(resource=resource_zipname)
    msg = textwrap_indent(msg)

    msg += "\n  For more information see: https://www.simple_nltk.org/data.html\n"
...
def retrieve(resource_url, filename=None, verbose=True):
    ...
    :param resource_url: A URL specifying where the resource should be
        loaded from.  The default protocol is "simple_nltk:", which searches
        for the file in the the simple_nltk data package.
    """
...
#: A dictionary describing the formats that are supported by simple_nltk's
...
FORMATS = {
    ...
    "parameter",
    "val": "A semantic valuation, parsed by simple_nltk.sem.Valuation.fromstring.",
...
def load(
    ...
    """
    Load a given resource from the simple_nltk data package.  The following
    ...
    if format == "raw":
        ...
        import json
        from simple_nltk.jsontags import json_tags
        ...
        if format == "text":
            ...
            resource_val = simple_nltk.grammar.CFG.fromstring(string_data, encoding=encoding)
        elif format == "pcfg":
            resource_val = simple_nltk.grammar.PCFG.fromstring(string_data, encoding=encoding)
        elif format == "fcfg":
            resource_val = simple_nltk.grammar.FeatureGrammar.fromstring(
                string_data,
            ...
            resource_val = simple_nltk.sem.read_logic(
                string_data,
                logic_parser=simple_nltk.sem.logic.LogicParser(),
                encoding=encoding,
            )
        elif format == "logic":
            resource_val = simple_nltk.sem.read_logic(
            ...
            resource_val = simple_nltk.sem.read_valuation(string_data, encoding=encoding)
        else:
            raise AssertionError(
                "Internal simple_nltk error: Format %s isn't "
                "handled by simple_nltk.data.load()" % (format,)
            )
...
def _open(resource_url):
    ...
    given its resource URL.  If the given resource URL uses the "simple_nltk:"
    protocol, or uses no protocol, then use ``simple_nltk.data.find`` to find
    ...
    if protocol is None or protocol.lower() == "simple_nltk":
