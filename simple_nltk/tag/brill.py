...
from simple_nltk.tag import TaggerI
from simple_nltk.tbl import Feature, Template
...
class Word(Feature):
    ...
    json_tag = "simple_nltk.tag.brill.Word"
...
class Pos(Feature):
    ...
    json_tag = "simple_nltk.tag.brill.Pos"
...
def simple_nltkdemo18():
    """
    Return 18 templates, from the original simple_nltk demo, in multi-feature syntax
    """
    return [
        Template(Pos([-1])),
        Template(Pos([1])),
...
def simple_nltkdemo18plus():
    """
    Return 18 templates, from the original simple_nltk demo, and additionally a few
    multi-feature ones (the motivation is easy comparison with simple_nltkdemo18)
    """
    return simple_nltkdemo18() + [
...
def fntbl37():
    ...
    fntbl can do that but the current simple_nltk implementation cannot.)
    """
    return [
...
class BrillTagger(TaggerI):
    ...
    json_tag = "simple_nltk.tag.BrillTagger"
