...
#
# Copyright (C) 2001-2013 simple_nltk Project
...
from simple_nltk.tag import untag, BrillTagger
...
class BrillTaggerTrainer(object):
    ...
    def train(self, train_sents, max_rules=200, min_score=2, min_acc=None):
        ...
        *min_acc*.
        ...
        >>> from simple_nltk.tag.brill import Pos, Word
        >>> from simple_nltk.tag import untag, RegexpTagger, BrillTaggerTrainer
        ...
        # NOTE1: (!!FIXME) A far better baseline uses simple_nltk.tag.UnigramTagger,
        # with a RegexpTagger only as backoff. For instance,
        # >>> baseline = UnigramTagger(baseline_data, backoff=backoff)
        # However, as of Nov 2013, simple_nltk.tag.UnigramTagger does not yield consistent results
