...
In the rest of the ``simple_nltk.classify`` module, the term "features" is
...
from simple_nltk.data import gzip_open_unicode
from simple_nltk.util import OrderedDict
from simple_nltk.probability import DictionaryProbDist

from simple_nltk.classify.api import ClassifierI
from simple_nltk.classify.util import CutoffChecker, accuracy, log_likelihood
from simple_nltk.classify.megam import call_megam, write_megam_file, parse_megam_weights
from simple_nltk.classify.tadm import call_tadm, write_tadm_file, parse_tadm_weights
...
def train_maxent_classifier_with_megam(
    ...
    :see: ``train_maxent_classifier()`` for parameter descriptions.
    :see: ``simple_nltk.classify.megam``
    """
    ...
    try:
        fd, trainfile_name = tempfile.mkstemp(prefix="simple_nltk-")
...
class TadmMaxentClassifier(MaxentClassifier):
    ...
    def train(cls, train_toks, **kwargs):
        ...
        trainfile_fd, trainfile_name = tempfile.mkstemp(
            prefix="simple_nltk-tadm-events-", suffix=".gz"
        )
        weightfile_fd, weightfile_name = tempfile.mkstemp(prefix="simple_nltk-tadm-weights-")
...
######################################################################
def demo():
    from simple_nltk.classify.util import names_demo
