...
# from simple_nltk.util import Deprecated
import simple_nltk.classify.util  # for accuracy & log_likelihood
...
class CutoffChecker(object):
    ...
    def check(self, classifier, train_toks):
        ...
        new_ll = simple_nltk.classify.util.log_likelihood(classifier, train_toks)
        ...
        if "max_acc" in cutoffs or "min_accdelta" in cutoffs:
            new_acc = simple_nltk.classify.util.log_likelihood(classifier, train_toks)
...
def names_demo(trainer, features=names_demo_features):
    from simple_nltk.corpus import names
    import random
...
def wsd_demo(trainer, word, features, n=1000):
    from simple_nltk.corpus import senseval
    import random
...
def check_megam_config():
    ...
    try:
        ...
        err_msg = str(
            ...
            ">>> simple_nltk.config_megam('/usr/bin/local/megam')"
        )
