...
optimization package. Before megam can be used, you should tell simple_nltk where it
can find the megam binary, using the ``config_megam()`` function. Typical
usage:

    >>> from simple_nltk.classify import megam
...
for details.
    ...
    simple_nltk.classify.MaxentClassifier.train(corpus, 'megam')
...
def config_megam(bin=None):
    """
    Configure simple_nltk's interface to the ``megam`` maxent optimization
    package.

    :param bin: The full path to the ``megam`` binary.  If not specified,
        then simple_nltk will search the system for a ``megam`` binary; and if
