...
"""
============================================
 TGrep search implementation for simple_nltk trees
============================================

This module supports TGrep2 syntax for matching parts of simple_nltk Trees.
...
>>> from simple_nltk.tree import ParentedTree
>>> from simple_nltk.tgrep import tgrep_nodes, tgrep_positions
...
This implementation adds syntax to select nodes based on their simple_nltk
...
- There are some issues with link relations involving leaf nodes
  (which are represented as bare strings in simple_nltk trees).  For
...
try:
    ...
    print("Warning: simple_nltk.tgrep will not work without the `pyparsing` package")
    print("installed.")

import simple_nltk.tree
...
def _istree(obj):
    """Predicate to check whether `obj` is a simple_nltk.tree.Tree."""
    return isinstance(obj, simple_nltk.tree.Tree)
...
def _tgrep_simple_nltk_tree_pos_action(_s, _l, tokens):
    """
    Builds a lambda function representing a predicate on a tree node
...
def _build_tgrep_parser(set_parse_actions=True):
    ...
    if set_parse_actions:
        ...
        tgrep_simple_nltk_tree_pos.setParseAction(_tgrep_simple_nltk_tree_pos_action)
...
def tgrep_positions(pattern, trees, search_leaves=True):
    ...
    :param trees: a sequence of simple_nltk trees (usually ParentedTrees)
