...
def config_java(bin=None, options=None, verbose=False):
    """
    Configure simple_nltk's java interface, by letting simple_nltk know where it can
    ...
    :param bin: The full path to the Java binary.  If not specified,
        then simple_nltk will search the system for a Java binary; and if
...
def read_str(s, start_position):
    ...
    :Example:
    >>> from simple_nltk.internals import read_str
...
def read_int(s, start_position):
    ...
    :Example:
    >>> from simple_nltk.internals import read_int
...
def read_number(s, start_position):
    ...
    :Example:
    >>> from simple_nltk.internals import read_number
...
def find_file_iter(
    ...
    """
    Search for a file to be used by simple_nltk.
    ...
    if not yielded:
        msg = (
            "simple_nltk was unable to find the %s file!"
...
def find_jar_iter(
    ...
    """
    Search for a jar that is used by simple_nltk.
    ...
    if not yielded:
        ...
        msg = "simple_nltk was unable to find %s!" % name_pattern
        if env_vars:
...
def import_from_stdlib(module):
    """
    When python is run from within the simple_nltk/ directory tree, the
    current directory is included at the beginning of the search path.
    Unfortunately, that means that modules within simple_nltk can sometimes
    shadow standard library modules.  As an example, the stdlib
    'inspect' module will attempt to import the stdlib 'tokenize'
    module, but will instead end up importing simple_nltk's 'tokenize' module
